...
#    np.savetxt('slowExplHex.csv', points, delimiter=',', fmt='%d')
    ...
    drawPentPrism()
    pentPoints = pentTransitionCurve()
#    np.savetxt('slowExplPent.csv', points, delimiter=',', fmt='%d')

    with open(OUTPUT_FILENAME, 'w') as f:
        f.write("// Slow explosive section polygons generated by explLenses.py")
        f.write("\n")
        f.write("hexPts = [\n")
        for pt in hexPoints:
            f.write(f"[{pt[0]}, {pt[1]}],\n")
